...
$gcd(a,1) = |a|$\tab $1$ ist Null von $gcd$ (Destruktor)\tab \tab ($a * 0 = 0$) ($a\: gcd \: 1 = 1$)\\
...
\\
\tab $\Rightarrow d \backslash (a-k*b) \wedge d\backslash b$\\
\\
\tab $<$ Def. $D_{a-k*b,b}$ $>$\\
\\
...
\uline{while} $x\neq y$ \uline{do}\\
\\
\uline{invariante} $gcd(x,y) = gcd(a,b) \wedge x>0 \wedge y>0$\\
\\
\uline{if} $x>y$ \uline{then}\\
\\
\tab $x:= x-y$\\
\\
\uline{else} // $x<y$ (assert $x<y$)\\
\\
\tab $y := y-x$\\
\\
\uline{endif}\\
